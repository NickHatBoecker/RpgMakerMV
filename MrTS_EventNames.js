...
* <EventNameQuestStart>
* <EventNameQuest: QUESTID>
* <EventNameQuestStep: STEPNUMBER>
*
...
* SIZE - Font Size
* EventNameQuestStart - Set this if it's the first event of the quest
*
* --------------------------------------------------------------------------------
* Plugin Commands
* --------------------------------------------------------------------------------
...
(function() {
    ...
    var regexEventNameQuestStart = /<EventNameQuestStart>/i;
    var regexEventNameQuest = /<EventNameQuest:(.*)>/i;
    var regexEventNameQuestStep = /<EventNameQuestStep:(.*)>/i;
    ...
    Game_System.prototype.setEventNamesNeedUpdate = function(value) {
        console.log(this);
    };
    ...
    Game_Event.prototype.initialize = function(mapId, eventId) {
        ...
        window.addEventListener('onQuestUpdate', function (e) {
            that.updateEventNameData();
        }, false);
    };
    ...
    Game_Event.prototype.resetEventNameData = function() {
        ...
        this._eventNameQuestStart = null;
        this._eventNameQuest = null;
        this._eventNameQuestStep = null;
    };
        ...
        if (this.list())
        {
            for (action of this.list())
            {
                if (action.code == "108" || action.code == "408") {
                    ...
                    var matchQuestStart = regexEventNameQuestStart.exec(a);
                    if (matchQuestStart) {
                        this._eventNameQuestStart = 1;
                        continue;
                    }
                    var matchQuest = regexEventNameQuest.exec(a);
                    if (matchQuest) {
                        this._eventNameQuest = matchQuest[1].trim();
                        continue;
                    }
                    var matchQuestStep = regexEventNameQuestStep.exec(a);
                    if (matchQuestStep) {
                        this._eventNameQuestStep = matchQuestStep[1].trim();
                        continue;
                    }
                }
            } // for
    ...
    Sprite_Character.prototype.updateEventNameData = function() {
        if (this._eventNameWindow && (this._character.getEventNameNeedUpdate() || this._eventNameWindow._needUpdate))
        {
            ...
            if (this.showConcerningQuest()) {
                ...
                this._eventNameWindow.setText(' ');
                this._eventNameWindow.setPicture(null);
            }
    ...
    Sprite_Character.prototype.showConcerningQuest = function() {
        var isQuestStart = this._character._eventNameQuestStart;
        var questId = this._character._eventNameQuest;
        var questStep = this._character._eventNameQuestStep;

        if (!questId) {
            // No quest mentioned, so show anyways
            return true;
        }

        if (isQuestStart && !$gameSystem.isQuestStarted(questId)) {
            // Show, because quest hasn't started yet
            return true;
        }

        if (questStep && $gameSystem.getQuestStep(questId) === questStep && !$gameSystem.isQuestFinished(questId)) {
            // Show, if current step and quest is not complete
            return true;
        }

        if (!questStep && !isQuestStart && $gameSystem.isQuestStarted(questId) && !$gameSystem.isQuestFinished(questId)) {
            // No step provided, so show if quest at least has started and is not the start event
            return true;
        }
